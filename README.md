...
Studio also has the first online-engine slices: one `GameProject` per game, a Phaser 3 starter template, a live preview that runs Phaser 3, a `main.ts` editor (transpiled, no imports), and **Build HTML5**. That build writes `engine.zip` (phaser.min.js + user.js + game.js) through the same scan pipeline as an uploaded release. It does not publish automatically. The visual scene editor and no-code events are later slices.
