...
Studio also has the first online-engine slice: one `GameProject` per game, a Phaser 3 starter template (`project.json` scenes/objects), a live preview, and **Build HTML5**. That build writes `engine.zip` through the same scan pipeline as an uploaded release. It does not publish automatically. TypeScript editing, the visual scene editor, and no-code events are later slices.
