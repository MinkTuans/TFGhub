...
for (const width of [390, 768, 1024, 1440]) {
  test(`catalog search and game information at ${width}px`, async ({ page }, testInfo) => {
    ...
    await page.getByRole("textbox", { name: "Tìm kiếm trò chơi" }).fill("no-such-game");
    await page.getByRole("button", { name: "Tìm kiếm", exact: true }).click();
    await expect(page.getByRole("heading", { name: "Chưa có trò chơi phù hợp." })).toBeVisible();
    await page.getByRole("link", { name: "Xem tất cả trò chơi", exact: true }).click();
    ...
    await expect(page.getByRole("heading", { name: "Thông tin trò chơi" })).toBeVisible();
...
test("related-game artwork leaves the available sidebar width for its text", async ({ page }) => {
  ...
  const card = page.getByRole("complementary", { name: "Trò chơi liên quan" }).getByRole("link").first();
