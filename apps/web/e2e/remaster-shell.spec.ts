...
for (const width of [390, 768, 1024, 1440]) {
  test(`Home and shell fit ${width}px with real catalog content`, async ({ page }, testInfo) => {
    ...
    await page.goto("/");
    await expect(page.getByRole("heading", { level: 1, name: "Mỗi trò chơi lớn đều bắt đầu từ một ý tưởng nhỏ." })).toBeVisible();
    ...
    if (width <= 768) {
      ...
      const toggle = page.getByRole("button", { name: "Mở trình đơn điều hướng" });
      await toggle.click();
