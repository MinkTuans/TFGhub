...
test.each([
  new ApiError(500, "Internal SQL details"),
  new ApiError(400, "toString"),
  new ApiError(400, "constructor"),
  new TypeError("offline"),
  new Error("Email already registered"),
  null,
])(
  "uses the caller's Vietnamese fallback for unknown or non-API failures",
  (error) => {
    expect(
      apiErrorMessage(error, "Không thể lưu hồ sơ. Vui lòng thử lại."),
    ).toBe("Không thể lưu hồ sơ. Vui lòng thử lại.");
  },
);
