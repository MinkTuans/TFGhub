...
Public eligible-game reads: `GET /engagement/games/:slug/stats` and `/comments?offset=0&limit=20`. Authenticated player reads their rating at `GET /engagement/games/:slug/me`, upserts it with `PUT .../rating {rating}` (204), removes it with `DELETE .../rating` (204), and creates a comment with `POST .../comments {body}` (201). Comment deletion at `DELETE .../comments/:commentId` (204) requires its author or MODERATOR/ADMIN.
